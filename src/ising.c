...
/*
 * Here we actually put j=boltzmann because of some reason (something about units probably)
 */
  ...
  for (i = 0; i < ntherm; i++)
	...
	if(!dataExists(par, tcorr, binders)) {
	  ...
	  result(par, vblock, 1, 1);
		...
		saveData(par,vblock, tcorrDat, magblock, tcorr, binders);
...
int read_args(Par *par, char *arg)
{
  ...
  *s++ = '\0';
  ...
  if (!strcmp(arg, "tcorr")) {
	...
	if (!strcmp(arg, "datadir")) {
		par->datadir = calloc(strlen(s), sizeof(char));
		strcpy(par->datadir,s);
		return 1;
	}
...
main(int argc, char *argv[])
{
  ...
  if (argc == 1) {
    printf("Usage: %s L=16 T=2.26 datadir=<datadir>\n", argv[0]);
